...
| Toy Domain Count | The total toy flag across the rulebook domains related to the project metadata. | _Order 3. Domains declared toys (Kind = toy)._ |
...
| Toy Share | Determined by priority: 0 if the domain count is 0; in all other cases, 100 times the toy domain count divided by the domain count rounded to 0 decimal place(s). | _Order 4. Percent of containers that are toys._ |
...
| Legacy Runner Capabilities | A defined attribute. | _Reverse relationship: legacy-runner capabilities inventoried for this project._ |
| Runner Capability Count | The number of legacy runner capabilities related to the project metadata. | _Order 1. Legacy-runner capabilities inventoried._ |
| Decided Capability Count | The total decided flag across the legacy runner capabilities related to the project metadata. | _Order 2. Capabilities with a decision and destination._ |
| Resolved Capability Count | The total resolved flag across the legacy runner capabilities related to the project metadata. | _Order 2. Capabilities whose successor exists and is wired._ |
| Is Runner Inventory Complete | True when all of the following hold: the runner capability count is greater than 0 and the decided capability count is the runner capability count. | _Order 3. Every inventoried capability has a recorded decision and destination (US-051)._ |
| Is Runner Succession Complete | True when all of the following hold: the runner inventory complete flag is set and the resolved capability count is the runner capability count. | _Order 4. Every capability's successor exists and is wired._ |
...
| Kind | A defined attribute. | _Declared classification: root \| toy \| example. A toy is a statement of intent (a small teaching rulebook), not a count of files; change this value to re-classify a project. The folder it lives in is witnessed separately as Area._ |
...
| Is Toy | True when the kind is “toy”. | _Order 1. Declared as a toy (Kind = toy)._ |
| Toy Flag | Determined by priority: 1 if the toy flag is set; in all other cases, 0. | _Order 2. 1 for declared toys — rollup carrier._ |
...
| Conformance Runs | A defined attribute. | _Reverse relationship: cross-substrate conformance harness runs recorded against this project._ |
...
| Conformance Run Count | The number of conformance runs related to the rulebook domain. | _Order 1b. Conformance harness runs recorded against this project._ |
...
| Expected Area | Determined by priority: “root” if the kind is “root”; “toy-rulebooks” if the toy flag is set; in all other cases, “rulebook-examples”. | _Order 2. Folder implied by the declared Kind._ |
...
| Summary | A defined attribute. | _What the capability does._ |
| Decision | A defined attribute. | _promote \| separate \| replace \| retire._ |
| Destination | A defined attribute. | _Who owns the behavior after retirement, or why nothing needs to._ |
| Rationale | A defined attribute. | _Why this decision._ |
| Status | A defined attribute. | _decided \| in-progress \| done. done means the successor exists and is wired; the runner keeps its copy regardless._ |
| Dependents | A defined attribute. | _Known consumers that must be repointed before removal._ |
| Project | A defined attribute. | _FK to ProjectMetadata._ |
| Is Decided | True when all of the following hold: the decision has a value and the destination has a value. | _Order 1. A decision and a destination are both recorded._ |
| Decided Flag | Determined by priority: 1 if all of the following hold: the decision has a value and the destination has a value; in all other cases, 0. | _Order 1. 1 when decided — rollup carrier._ |
| Is Kept | True when at least one of the following holds: the decision is “promote” or the decision is “separate”. | _Order 1. The platform still needs the behavior (promote or separate)._ |
| Is Resolved | True when the status is “done”. | _Order 1. The decision has been carried out._ |
...
| Status | A defined attribute. | _last_run.status from the harness: success or failure._ |
| Score | A defined attribute. | _last_run.score (0-100) from the harness._ |
| Fields Tested | A defined attribute. | _last_successful_run.test_results.total_fields_tested._ |
| Fields Passed | A defined attribute. | _last_successful_run.test_results.fields_passed._ |
| Fields Failed | A defined attribute. | _last_successful_run.test_results.fields_failed._ |
| Duration Seconds | A defined attribute. | _last_run.duration_seconds._ |
...
| Registration State | Determined by priority: “unregistered” if the registered flag is not set; “no-test-files” if the test file count is 0, in all other cases “ready” if the suite kind is “pytest”; “no-effortless-json” if the effortless JSON flag is not set; “never-exercised” if the answer key count is 0; in all other cases, “ready”. | _Order 1. Why this suite will or will not run: unregistered \| no-effortless-json \| never-exercised (runnable, but the first run generates its keys) \| no-test-files \| ready._ |
...
| Test File Count | A defined attribute. | _WITNESSED: test_*.py files in a pytest suite's directory. Zero means the suite is registered but has nothing to run — which is how a platform test suite silently disappearing becomes visible in the registry instead of only as a red run._ |
...
| Is Scanner Derived | True when an empty string. | _True when this rule's findings are re-derived by scripts/scan-project-slots.py; they close by re-running the scan, never by hand._ |
| Project | A defined attribute. | _FK to ProjectMetadata._ |
...
| Rule is Scanner Derived | True when the linked rule is scanner derived. | _Order 1. Whether the violated rule's findings are scanner-derived._ |
...
| Is Hand Closable | True when all of the following hold: the open flag is set and the rule is scanner derived flag is not set. | _Order 2. Open and not scanner-derived: the work queue may mark it fixed or accepted-exception by hand._ |
...
| Domain Kind | Taken from the linked domain. | _Order 1. The declared Kind (root \| toy \| example) of the witnessed project._ |
...
| **DR-27 Decided Capability Count** | A project metadata's decided capability count is the total decided flag across the legacy runner capabilities related to the project metadata. |
| **DR-28 Resolved Capability Count** | A project metadata's resolved capability count is the total resolved flag across the legacy runner capabilities related to the project metadata. |
| **DR-29 Is Runner Inventory Complete** | A project metadata is considered a runner inventory complete if all of the following hold: the runner capability count is greater than 0 and the decided capability count is the runner capability count. |
| **DR-30 Is Runner Succession Complete** | A project metadata is considered a runner succession complete if all of the following hold: the runner inventory complete flag is set and the resolved capability count is the runner capability count. |
| **DR-31 Name** | An ontology axiom's name is the same as its short name. |
| **DR-32 Invariant Count** | An ontology axiom's invariant count is the number of framing invariants related to the ontology axiom. |
| **DR-33 Feature Count** | An ontology axiom's feature count is the number of platform features related to the ontology axiom. |
| **DR-34 Is Active** | An ontology axiom is considered active if the status is “active”. |
| **DR-35 Critical Invariant Count** | An ontology axiom's critical invariant count is the total critical flag across the framing invariants related to the ontology axiom. |
| **DR-36 Shipped Feature Count** | An ontology axiom's shipped feature count is the total shipped flag across the platform features related to the ontology axiom. |
| **DR-37 Is Load Bearing** | An ontology axiom is considered load-bearing if at least one of the following holds: the invariant count is greater than 0 or the feature count is greater than 0. |
| **DR-38 Is Well Guarded** | An ontology axiom is considered well-guarded if all of the following hold: the load bearing flag is set and the critical invariant count is greater than 0. |
...
| **DR-50 Needs Attention** | A framing invariant is considered to need an attention if the enforcement state is “orphan”. |
| **DR-51 Is Readme Stub** | A platform feature is considered a readme stub if the readme length (a missing value counts as 0) is less than 400. |
| **DR-52 Is Headline** | A platform feature is considered a headline if the tier is “headline”. |
| **DR-53 Is Shipped** | A platform feature is considered shipped if the status is “shipped”. |
| **DR-54 Shipped Flag** | The platform feature's shipped flag is determined by the following priority:<br>1. 1, if the status is “shipped”;<br>2. in all other cases, 0. |
| **DR-55 Axiom Short Name** | A platform feature's axiom short name — taken from the linked related axiom ID. |
| **DR-56 Needs Readme Work** | A platform feature is considered to need a readme work if all of the following hold: the shipped flag is set and the readme stub flag is set. |
| **DR-57 Axiom Invariant Count** | A platform feature's axiom invariant count — taken from the linked related axiom ID. |
| **DR-58 Axiom is Load Bearing** | A platform feature's axiom is load bearing when the linked related axiom ID is load bearing. |
| **DR-59 Is Headline Gap** | A platform feature is considered a headline gap if all of the following hold: the headline flag is set and the needs readme work flag is set. |
...
| **DR-69 Name** | A rulebook domain's name is the same as its domain name. |
| **DR-70 Slug** | A rulebook domain's slug is computed as 8 character(s) of the domain ID starting at position 200. |
| **DR-71 Is Toy** | A rulebook domain is considered a toy if the kind is “toy”. |
| **DR-72 Toy Flag** | The rulebook domain's toy flag is determined by the following priority:<br>1. 1, if the toy flag is set;<br>2. in all other cases, 0. |
| **DR-73 Has Rulebook** | A rulebook domain is considered to have a rulebook if the rulebook path has a value. |
| **DR-74 Finding Count** | A rulebook domain's finding count is the number of consistency findings related to the rulebook domain. |
| **DR-75 Flavor Card Count** | A rulebook domain's flavor card count is the number of rulebook flavors related to the rulebook domain. |
| **DR-76 Child Domain Count** | A rulebook domain's child domain count is the number of rulebook domains related to the rulebook domain. |
| **DR-77 Expected Rulebook Path** | A rulebook domain's expected rulebook path is computed as the relative path, followed by “effortless-rulebook/”, followed by the slug, followed by “-rulebook.json”. |
| **DR-78 Open Finding Count** | A rulebook domain's open finding count is the total open flag across the consistency findings related to the rulebook domain. |
...
| **DR-93 Conformance Run Count** | A rulebook domain's conformance run count is the number of conformance runs related to the rulebook domain. |
| **DR-94 Present Slot Count** | A rulebook domain's present slot count is the total present flag across the project slot witnesses related to the rulebook domain. |
| **DR-95 Implementation Gap Count** | A rulebook domain's implementation gap count is the total implementation gap flag across the project slot witnesses related to the rulebook domain. |
| **DR-96 Universal Gap Count** | A rulebook domain's universal gap count is the total universal gap flag across the project slot witnesses related to the rulebook domain. |
| **DR-97 Slot Coverage Percent** | The rulebook domain's slot coverage percent is determined by the following priority:<br>1. 0, if the slot witness count is 0;<br>2. in all other cases, 100 times the present slot count divided by the slot witness count rounded to 0 decimal place(s). |
| **DR-98 Required Slot Count** | A rulebook domain's required slot count is the total required here flag across the project slot witnesses related to the rulebook domain. |
| **DR-99 Required Present Count** | A rulebook domain's required present count is the total required present flag across the project slot witnesses related to the rulebook domain. |
| **DR-100 Required Gap Count** | A rulebook domain's required gap count is the total gap flag across the project slot witnesses related to the rulebook domain. |
| **DR-101 Is Fully Implemented** | A rulebook domain is considered fully-implemented if all of the following hold: the implementation gap count is 0 and the intentional exception flag is not set. |
| **DR-102 Fully Implemented Flag** | The rulebook domain's fully implemented flag is determined by the following priority:<br>1. 1, if all of the following hold: the implementation gap count is 0 and the intentional exception flag is not set;<br>2. in all other cases, 0. |
| **DR-103 Required Slot Coverage Percent** | The rulebook domain's required slot coverage percent is determined by the following priority:<br>1. 100, if the required slot count is 0;<br>2. in all other cases, 100 times the required present count divided by the required slot count rounded to 0 decimal place(s). |
...
| **DR-149 Is Runnable** | A test suite is considered runnable if all of the following hold: the registered flag is set and at least one of the following holds: all of the following hold: the suite kind is “pytest” and the test file count is greater than 0 or all of the following hold: the suite kind is not “pytest” and the effortless JSON flag is set. |
| **DR-150 Runnable Flag** | The test suite's runnable flag is determined by the following priority:<br>1. 1, if all of the following hold: the registered flag is set and at least one of the following holds: all of the following hold: the suite kind is “pytest” and the test file count is greater than 0 or all of the following hold: the suite kind is not “pytest” and the effortless JSON flag is set;<br>2. in all other cases, 0. |
...
| **DR-153 Registration State** | The test suite's registration state is determined by the following priority:<br>1. “unregistered”, if the registered flag is not set;<br>2. “no-test-files” if the test file count is 0, in all other cases “ready”, if the suite kind is “pytest”;<br>3. “no-effortless-json”, if the effortless JSON flag is not set;<br>4. “never-exercised”, if the answer key count is 0;<br>5. in all other cases, “ready”. |
...
| **ProjectMetadata.RunnerCapabilityCount** | rollup | `Count(LegacyRunnerCapabilities via Project)` |
| **ProjectMetadata.DecidedCapabilityCount** | rollup | `Sum(LegacyRunnerCapabilities.DecidedFlag via Project)` |
| **ProjectMetadata.ResolvedCapabilityCount** | rollup | `Sum(LegacyRunnerCapabilities.ResolvedFlag via Project)` |
| **ProjectMetadata.IsRunnerInventoryComplete** | formula | `And(RunnerCapabilityCount > 0, DecidedCapabilityCount = RunnerCapabilityCount)` |
| **ProjectMetadata.IsRunnerSuccessionComplete** | formula | `And(IsRunnerInventoryComplete, ResolvedCapabilityCount = RunnerCapabilityCount)` |
...
| **RulebookDomains.IsToy** | formula | `Kind = "toy"` |
| **RulebookDomains.ToyFlag** | formula | `If(IsToy, 1, 0)` |
...
| **RulebookDomains.ConformanceRunCount** | rollup | `Count(ConformanceRuns via Domain)` |
...
| **LegacyRunnerCapabilities.Name** | formula | `Title` |
| **LegacyRunnerCapabilities.IsDecided** | formula | `And(Decision <> "", Destination <> "")` |
| **LegacyRunnerCapabilities.DecidedFlag** | formula | `If(And(Decision <> "", Destination <> ""), 1, 0)` |
| **LegacyRunnerCapabilities.IsKept** | formula | `Or(Decision = "promote", Decision = "separate")` |
| **LegacyRunnerCapabilities.IsResolved** | formula | `Status = "done"` |
| **LegacyRunnerCapabilities.ResolvedFlag** | formula | `If(Status = "done", 1, 0)` |
| **LegacyRunnerCapabilities.CapabilityState** | formula | `If(Not(IsDecided), "undecided", If(IsResolved, "resolved", "decided"))` |
| **LegacyRunnerCapabilities.CapabilityLabel** | formula | `Concat(Title, " [", Decision, "]")` |
| **ConformanceRuns.Name** | formula | `ConformanceRunId` |
| **ConformanceRuns.DomainName** | lookup | `Lookup(RulebookDomains.DomainName via Domain)` |
| **ConformanceRuns.TotalSubstrates** | rollup | `Count(ConformanceResults via Run)` |
| **ConformanceRuns.PassingSubstrateCount** | rollup | `Sum(ConformanceResults.IsPassingFlag via Run)` |
| **ConformanceRuns.FailingSubstrateCount** | formula | `TotalSubstrates - PassingSubstrateCount` |
| **ConformanceRuns.OverallScore** | rollup | `Average(ConformanceResults.Score via Run)` |
| **ConformanceRuns.OverallStatus** | formula | `If(TotalSubstrates = 0, "no-results", If(FailingSubstrateCount = 0, "all-passing", "has-failures"))` |
| **ConformanceResults.Name** | formula | `ConformanceResultId` |
...
| **TestSuites.IsRunnable** | formula | `And(IsRegistered, Or(And(SuiteKind = "pytest", TestFileCount > 0), And(SuiteKind <> "pytest", HasEffortlessJson)))` |
| **TestSuites.RunnableFlag** | formula | `If(And(IsRegistered, Or(And(SuiteKind = "pytest", TestFileCount > 0), And(SuiteKind <> "pytest", HasEffortlessJson))), 1, 0)` |
...
| **TestSuites.RegistrationState** | formula | `If(Not(IsRegistered), "unregistered", If(SuiteKind = "pytest", If(TestFileCount = 0, "no-test-files", "ready"), If(Not(HasEffortlessJson), "no-effortless-json", If(AnswerKeyCount = 0, "never-exercised", "ready"))))` |
...
| **ConsistencyFindings.RuleIsScannerDerived** | lookup | `Lookup(ConsistencyRules.IsScannerDerived via Rule)` |
...
| **ConsistencyFindings.IsHandClosable** | formula | `And(IsOpen, Not(RuleIsScannerDerived))` |
...
| **ProjectSlotWitnesses.DomainKind** | lookup | `Lookup(RulebookDomains.Kind via Domain)` |
| **ProjectSlotWitnesses.DomainIsException** | lookup | `Lookup(RulebookDomains.IsIntentionalException via Domain)` |
| **ProjectSlotWitnesses.IsRequiredHere** | formula | `And(Not(Coalesce(DomainIsException, False())), Or(And(DomainKind = "root", Coalesce(SlotRequiredForRoot, False())), And(DomainKind = "example", Coalesce(SlotRequiredForExample, False())), And(DomainKind = "toy", Coalesce(SlotRequiredForToy, False()))))` |
| **ProjectSlotWitnesses.ImplementationGapFlag** | formula | `If(And(Not(IsPresent), Not(Coalesce(DomainIsException, False())), Or(And(DomainKind = "root", Coalesce(SlotRequiredForRoot, False())), And(DomainKind <> "root", Coalesce(SlotRequiredForExample, False())))), 1, 0)` |

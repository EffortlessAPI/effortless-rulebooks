...
RETURNS TEXT AS $$
  SELECT (CONCAT((SELECT NULLIF(last_name, '') FROM customers WHERE customer_id = p_customer_id), ', ', (SELECT NULLIF(first_name, '') FROM customers WHERE customer_id = p_customer_id), ' - v6'))::text;
$$ LANGUAGE sql STABLE;

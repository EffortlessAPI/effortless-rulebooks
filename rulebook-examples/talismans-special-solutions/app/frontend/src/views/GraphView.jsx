...
export function GraphView({ sit, handlers }) {
  ...
  const isLit = (id) => !focus || id === focus || neighbors[focus]?.has(id);
  const edgeLit = (e) => !focus || e.from === focus || e.to === focus;
...
function GNode({ node, x, y, lit, focused, onClick }) {
  if (node.type === "step") {
    const k = KIND[node.agentKind] || KIND.unknown;
    return (
  ...
  // agent
  const k = KIND[node.kind] || KIND.unknown;
  return (
...
function FocusPanel({ sit, focus }) {
  ...
  const nm = (id) => sit.graph.nodes.find((n) => n.id === id)?.label || id;
  return (
...
function positions(sit) {
  ...
  agentNodes.forEach((a) => {
    const xs = sit.graph.edges
      ...
      .map((e) => pos[e.to]?.x).filter((v) => v != null);

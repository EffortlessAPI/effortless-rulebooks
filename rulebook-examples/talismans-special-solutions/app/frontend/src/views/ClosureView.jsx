...
import { kindOfType } from "../model.js";
    ...
    <div className="closure">
      <div className="cl-intro">
        ...
        <div className="cl-counts">
          ...
          <span className="cl-count total"><b>{sit.closure?.count ?? pairs.length}</b> total ordered pairs</span>
        </div>
      </div>
      ...
      <div className="cl-chainwrap">
            ...
            return (
              <React.Fragment key={s.id}>
                <div
                  ...
                  onDragOver={(e) => { if (dragFrom && dragFrom !== s.id) { e.preventDefault(); setHoverTo(s.id); } }}
                  onDragLeave={() => setHoverTo((h) => (h === s.id ? null : h))}
                  onDrop={(e) => { e.preventDefault(); tryAssert(dragFrom, s.id); }}
                >
                  ...
                  <div
                    className="cl-handle"
                    draggable
                    onDragStart={(e) => { e.dataTransfer.effectAllowed = "link"; setDragFrom(s.id); }}
        ...
        <div className="cl-asserted-row">
          ...
          {asserted
            .slice()
            .sort((a, b) => posOf(a.from) - posOf(b.from) || posOf(a.to) - posOf(b.to))
            .map((e) => (
...
function ClosureMatrix({ steps, pairs, assertedSet, posOf }) {
...
function firstChainHint(asserted, posOf) {
  if (!asserted.length) return "";
  const e = asserted
    .slice()
    .sort((a, b) => posOf(a.from) - posOf(b.from))[0];

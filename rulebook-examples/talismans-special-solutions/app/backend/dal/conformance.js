// ---------------------------------------------------------------------------
// conformance.js — the live differential between the two substrates.
//
...
function indexByPk(rows, pkCamel) {
  const m = new Map();
...
export function diffPayloads(rs, pg, pkByClass, fieldTypeByClass = {}) {
  const diffs = [];
  ...
  const classesOnlyIn = { reasoner: [], postgres: [] };

  for (const cls of allClasses) {
    const rsRows = rs.individuals[cls];
    const pgRows = pg.individuals[cls];
...
export function fieldTypesFromSchemaMap(schemaMap) {
  const out = {};
...
export async function runConformance(db, pkByClass, fieldTypeByClass) {

...
VALUES ('prod-deploy-step-5', 'Post-Deployment Health Check & Report', 'production-deployment', 5, 'ntwf-deployment-health-role', FALSE, 105, '', 'artifact-post-deploy-report', '', '', 'prec-4-5') ON CONFLICT (workflow_step_id) DO NOTHING;
...
INSERT INTO roles (role_id, display_name, label, comment, has_capability, filled_by_human_agent, filled_by_ai_agent, filled_by_automated_pipeline, owned_by, delegates_to, workflow_steps, from_delegates_to)
VALUES ('ntwf-risk-analysis-role', 'Risk Analysis Agent', 'Risk Analysis Agent', 'Performs probabilistic risk scoring over the Q1 risk metrics dataset. This is a role, not an identity — filled by RiskAnalysis-AI today but could be replaced by a newer model version without changing the workflow structure.', 'cap-risk-analysis', 'ntwf-james-okafor', '', '', 'ntwf-engineering', '', 'prod-deploy-step-1', '') ON CONFLICT (role_id) DO NOTHING;

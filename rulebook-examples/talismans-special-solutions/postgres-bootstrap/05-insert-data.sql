...
-- ----------------------------------------------------------------------------
INSERT INTO workflow_steps (workflow_step_id, display_name, workflow, assigned_role, requires_human_approval, step_duration_minutes, consumes_dataset, produces_artifacts, requires_artifacts, approval_gate, precedes, preceded_by)
VALUES ('prod-deploy-step-3', 'Release Approval Gate', 'production-deployment', 'ntwf-release-manager-role', TRUE, 30, '', 'artifact-release-authorization', 'artifact-legal-clearance', 'ntwf-release-approval-gate', 'prec-3-4', 'prec-2-3') ON CONFLICT (workflow_step_id) DO NOTHING;

INSERT INTO workflow_steps (workflow_step_id, display_name, workflow, assigned_role, requires_human_approval, step_duration_minutes, consumes_dataset, produces_artifacts, requires_artifacts, approval_gate, precedes, preceded_by)
VALUES ('prod-deploy-step-2', 'Legal Compliance Review', 'production-deployment', 'ntwf-legal-compliance-role', TRUE, 120, '', 'artifact-legal-clearance', 'artifact-risk-report', '', 'prec-2-3', 'prec-1-2') ON CONFLICT (workflow_step_id) DO NOTHING;

INSERT INTO workflow_steps (workflow_step_id, display_name, workflow, assigned_role, requires_human_approval, step_duration_minutes, consumes_dataset, produces_artifacts, requires_artifacts, approval_gate, precedes, preceded_by)
VALUES ('prod-deploy-step-5', 'Post-Deployment Health Check & Report', 'production-deployment', 'ntwf-deployment-health-role', FALSE, 60, '', 'artifact-post-deploy-report', '', '', '', 'prec-4-5') ON CONFLICT (workflow_step_id) DO NOTHING;

INSERT INTO workflow_steps (workflow_step_id, display_name, workflow, assigned_role, requires_human_approval, step_duration_minutes, consumes_dataset, produces_artifacts, requires_artifacts, approval_gate, precedes, preceded_by)
VALUES ('prod-deploy-step-1', 'AI Risk Assessment', 'production-deployment', 'ntwf-risk-analysis-role', FALSE, 15, 'ds-q1-2026-risk-metrics', 'artifact-risk-report', '', '', 'prec-1-2', '') ON CONFLICT (workflow_step_id) DO NOTHING;

INSERT INTO workflow_steps (workflow_step_id, display_name, workflow, assigned_role, requires_human_approval, step_duration_minutes, consumes_dataset, produces_artifacts, requires_artifacts, approval_gate, precedes, preceded_by)
VALUES ('prod-deploy-step-4', 'Automated Deployment Execution', 'production-deployment', 'ntwf-ci-executor-role', FALSE, 45, '', 'artifact-deployment-log', 'artifact-release-authorization', '', 'prec-4-5', 'prec-3-4') ON CONFLICT (workflow_step_id) DO NOTHING;
...
INSERT INTO conformance_tests (conformance_test_id, display_name, feature_ref, section, test_kind, target_ref, expect, explanation, sort_order, is_enabled)
VALUES ('sequence-position', 'Step 4 sequence position', 'III-sequencePosition', 'Part III', 'field-match', 'WorkflowSteps/prod-deploy-step-4#SequencePosition', '', 'Step position is DERIVED from the precedence closure (PrecedingStepCount + 1), not a hand-asserted ordinal; step 4 resolves to 4 in both substrates because four steps transitively precede it.', 480, TRUE) ON CONFLICT (conformance_test_id) DO NOTHING;

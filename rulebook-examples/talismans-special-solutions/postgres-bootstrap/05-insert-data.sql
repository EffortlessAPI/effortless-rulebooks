...
VALUES ('ds-q1-2026-risk-metrics', 'Q1 2026 Risk Metrics', 'DS-RISK-2026-Q1', '2026-03-31T00:00:00-05:00', 'https://data.internal.special-solutions.example/risk/q1-2026', 'prod-deploy-step-1') ON CONFLICT (dataset_id) DO NOTHING;
...
INSERT INTO scenarios (scenario_id, label, icon, explanation, sort_order, is_reset, edits)
VALUES ('dataset-unconsumed', 'Detach the risk dataset', '📦', 'Clears the AI Risk Assessment step''s consumes-dataset link (WorkflowSteps.ConsumesDataset is the authoritative DCAT forward FK; Datasets.ConsumedBySteps is its derived inverse). The Q1 Risk Metrics dataset is now consumed by no step — on the Graph lens its 📦 node visibly detaches from step 1. Demonstrates CQ8: dataset consumption is a real relationship, and the answer is exactly which steps point at it; with nothing consuming it, the AI-provenance answer is gone.', 80, FALSE, '[{"class": "WorkflowSteps", "id": "prod-deploy-step-1", "set": {"consumesDataset": ""}}]') ON CONFLICT (scenario_id) DO NOTHING;

...
-- ----------------------------------------------------------------------------
INSERT INTO competency_questions (competency_question_id, number, display_name, question_text, target_table, target_field, answer_kind, expected_answer, explanation, sort_order, is_active)
VALUES ('cq-1', 1, 'Steps and order', 'What are all the steps in the release process, and in what order do they execute?', 'Workflows', 'CountOfPrecedenceClosurePairs', 'list', '5 steps, ordered 1 -> 2 -> 3 -> 4 -> 5 (10 closure pairs)', 'WorkflowSteps ordered by SequencePosition; the order is the transitive closure of the asserted StepPrecedence edges, materialized as ntwf:precedesStep.', 1, TRUE) ON CONFLICT (competency_question_id) DO NOTHING;

INSERT INTO competency_questions (competency_question_id, number, display_name, question_text, target_table, target_field, answer_kind, expected_answer, explanation, sort_order, is_active)
VALUES ('cq-2', 2, 'Who approves', 'Who or what is responsible for approving a production deployment?', 'ApprovalGates', 'GateApproverHuman', 'scalar', 'Maria Gonzalez', 'The Release Approval Gate (step 3) -> its AssignedRole (Release Manager) -> FilledByHumanAgent. The role-agent indirection is resolved by the GateApproverHuman lookup.', 2, TRUE) ON CONFLICT (competency_question_id) DO NOTHING;

INSERT INTO competency_questions (competency_question_id, number, display_name, question_text, target_table, target_field, answer_kind, expected_answer, explanation, sort_order, is_active)
VALUES ('cq-3', 3, 'AI vs human steps', 'Which steps are executed by AI agents, and which require human decision?', 'Workflows', 'CountAISteps', 'list', 'AI-executed: steps 1, 5 - human-required: steps 2, 3', 'Each step''s ExecutingAgentType (via its role''s filler type) classifies AI vs pipeline vs human; RequiresHumanApproval marks the human-decision steps. CountAISteps aggregates the AI side.', 3, TRUE) ON CONFLICT (competency_question_id) DO NOTHING;

INSERT INTO competency_questions (competency_question_id, number, display_name, question_text, target_table, target_field, answer_kind, expected_answer, explanation, sort_order, is_active)
VALUES ('cq-4', 4, 'Artifact lineage', 'What artifacts does the review produce, and what consumes them downstream?', 'WorkflowArtifacts', 'DerivedFromArtifact', 'list', '5 artifacts in a wasDerivedFrom chain: Risk Report -> Legal Clearance -> Release Authorization -> Deployment Log -> Post-Deployment Report', 'WorkflowArtifacts.ProducedByStep gives the producer; DerivedFromArtifact gives the PROV wasDerivedFrom parent; RequiredBySteps gives the downstream consumer.', 4, TRUE) ON CONFLICT (competency_question_id) DO NOTHING;

INSERT INTO competency_questions (competency_question_id, number, display_name, question_text, target_table, target_field, answer_kind, expected_answer, explanation, sort_order, is_active)
VALUES ('cq-5', 5, 'Stale over 12 months', 'Which workflows have not been reviewed or updated in twelve months?', 'Workflows', 'IsStale', 'scalar', 'No - Production Deployment is not stale (MonthsSinceModified 2 <= 12)', 'IsStale = MonthsSinceModified > StalenessThresholdMonths. The seed workflow was modified 2 months ago against a 12-month threshold, so it is not stale.', 5, TRUE) ON CONFLICT (competency_question_id) DO NOTHING;

INSERT INTO competency_questions (competency_question_id, number, display_name, question_text, target_table, target_field, answer_kind, expected_answer, explanation, sort_order, is_active)
VALUES ('cq-6', 6, 'Escalation path', 'What happens when the VP of Engineering is unavailable during escalation?', 'Roles', 'DelegatesTo', 'scalar', 'Escalates Release Manager -> VP of Engineering -> Chief Technology Officer', 'Roles.DelegatesTo is a transitive chain. With the VP of Engineering unavailable, the next hop in the closure is the CTO.', 6, TRUE) ON CONFLICT (competency_question_id) DO NOTHING;

INSERT INTO competency_questions (competency_question_id, number, display_name, question_text, target_table, target_field, answer_kind, expected_answer, explanation, sort_order, is_active)
VALUES ('cq-7', 7, 'Engineering and Legal', 'Which workflows involve both Engineering and Legal?', 'Workflows', 'InvolvesEngineeringAndLegal', 'scalar', 'Yes - Production Deployment involves both (Engineering-owned steps and Legal-owned steps)', 'InvolvesEngineeringAndLegal = AND(CountEngineeringOwnedSteps > 0, CountLegalOwnedSteps > 0). Step 2 is Legal-owned; steps 1/3/4/5 are Engineering-owned.', 7, TRUE) ON CONFLICT (competency_question_id) DO NOTHING;

INSERT INTO competency_questions (competency_question_id, number, display_name, question_text, target_table, target_field, answer_kind, expected_answer, explanation, sort_order, is_active)
VALUES ('cq-8', 8, 'Datasets and AI', 'What datasets does the review consume, and which AI agents processed them?', 'Datasets', 'ConsumedBySteps', 'list', 'Q1 2026 Risk Metrics, consumed by the AI Risk Assessment step and processed by RiskAnalysis-AI', 'Datasets.ConsumedBySteps (inverse of WorkflowSteps.ConsumesDataset) gives the consuming step; that step''s role filler resolves the processing AI agent.', 8, TRUE) ON CONFLICT (competency_question_id) DO NOTHING;

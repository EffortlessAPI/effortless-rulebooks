...
For this project, **Airtable base `app7G5emeY7miM4WN` is the single, authoritative source of truth.** Edit the schema and data in Airtable. The local `effortless-rulebook/nakedclaude-v2-rulebook.json` is a generated IR — every `effortless build` pulls fresh from Airtable and overwrites it.
...
- **Do not hand-edit `nakedclaude-v2-rulebook.json` and expect changes to stick.** The next `effortless build` will overwrite them with whatever is in Airtable.

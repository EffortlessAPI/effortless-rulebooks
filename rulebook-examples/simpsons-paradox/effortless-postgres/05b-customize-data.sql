...
VALUES
  ...
  ('loop-41', 'ModelSummary expansion: ratio statistics per type and InstrumentScore summary', 'complete', 'MinRatioTypeA, MaxRatioTypeA, MinRatioTypeB, AvgInstrumentScoreDanger, DangerTierCount, SafeTierCount', 'What do DistortionRatio statistics reveal about the range within each type, and how many studies fall in each tier?', 'loop-42: InstrumentDashboard UI — tier-colored table view of all 16 studies'),
  ('loop-42', 'InstrumentDashboard: UI view showing DistortionRatio, InstrumentScore, ScreeningTier across all 16 studies', 'complete', 'InstrumentDashboardView', 'Can a single table view make the instrument''s screening output readable at a glance?', 'loop-43: C+/C- split — type-C collapses two opposite errors; split into amplification (C+) and compression (C-) with distinct invariants.'),

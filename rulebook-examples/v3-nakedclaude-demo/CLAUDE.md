...
## Authoritative Source: Airtable base `appKLygCIXweUUKtM` is HEAD

For this project, **Airtable base `appKLygCIXweUUKtM` is the single, authoritative source of truth.** Edit the schema and data in Airtable. The local `effortless-rulebook/v3-nakedclaude-demo-rulebook.json` is a generated IR — every `effortless build` pulls fresh from Airtable and overwrites it.
...
- **Do not hand-edit `v3-nakedclaude-demo-rulebook.json` and expect changes to stick.** The next `effortless build` will overwrite them with whatever is in Airtable.

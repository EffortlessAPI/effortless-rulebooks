...
DATABASE_URL="${DATABASE_URL:-postgresql://postgres@localhost:5432/product_inventory_demo}"
...
case "${1:-all}" in
  ...
  *)
    ...
    echo "  effortless build           - regenerate postgres/"
    echo "  ./postgres/init-db.sh      - drop/recreate DB and apply schema"
    echo ""

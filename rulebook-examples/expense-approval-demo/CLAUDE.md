# Expense Approval Demo — Effortless Project

<!-- rulebook-authoritative-banner -->
## Authoritative Source: `effortless-rulebook/expense-approval-demo-rulebook.json` is HEAD

For this project, **`effortless-rulebook/expense-approval-demo-rulebook.json` is the single, authoritative source of truth.** Edit it directly. All other artifacts (Postgres SQL, optional substrates) are mechanically derived from it via `effortless build`.
...
### Never silently revert `expense-approval-demo-rulebook.json`

**Treat `effortless-rulebook/expense-approval-demo-rulebook.json` as sacred.** It contains human-authored business rules and CANNOT be casually overwritten. Before running any command that could touch this file — `effortless build`, any sync script, any `git checkout`/`git restore` against it — first run `git status` / `git diff` on it.
...
**Location:** `effortless-rulebook/expense-approval-demo-rulebook.json`
...
Edit `effortless-rulebook/expense-approval-demo-rulebook.json` directly, then rebuild:
...
| `CLAUDE.md` | This file |
| `effortless-rulebook/expense-approval-demo-rulebook.json` | The rulebook (SSoT) |

...
def validate_squashed_calibration_ledger() -> None:
    ...
    if mode.get('detailed_git_commit_history_required') is not False:
        raise AssertionError('squashed history mode must disable commit-subject dependence')
    ...
    if missing:
        raise AssertionError(f'canonical calibration loops missing: {missing}')
    ...
    print('traveling-salesman calibration canonical-ledger validation: PASS')

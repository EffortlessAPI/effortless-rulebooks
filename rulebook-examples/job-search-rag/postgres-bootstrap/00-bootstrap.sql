...
-- ============================================================================
-- DROP ALL OBJECTS - Clean slate for database (ENABLED: destructive rebuild)
-- ============================================================================
-- Generated by rulebook-to-postgres tool
-- Drops every view (CASCADE), then every function (CASCADE), then every base
-- table (CASCADE) in the public schema. init-db.sh runs this FIRST, so the
-- subsequent 01..05 files recreate tables, functions, views, and re-insert the
-- rulebook's data into now-empty tables. This guarantees raw facts (incl. PKs)
-- always match the rulebook — no stale rows survive an ON CONFLICT DO NOTHING.
-- ============================================================================
...
BEGIN
    ...
    FOR r IN (SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_type = 'BASE TABLE') LOOP
        ...
        v_count := v_count + 1;
    END LOOP;
    RAISE NOTICE 'Dropped % tables', v_count; v_count := 0;

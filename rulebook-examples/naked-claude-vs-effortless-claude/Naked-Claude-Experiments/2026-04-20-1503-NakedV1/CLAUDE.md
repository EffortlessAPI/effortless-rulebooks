...
- A `start.sh` at the repo root that, when run, brings up the app
  ...
  local `effortless-demo` postgres, start the backend, start the frontend
  build/dev server in the foreground or via clearly-tagged background
  processes that the script can also stop). The grader will run
  `./start.sh` to test the app — make it just work.

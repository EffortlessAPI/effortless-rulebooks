...
1. `cd rulebook-examples/expense-approval`
2. `effortless -init`
3. Author the rulebook from the existing `README.md` spec
4. Save it as `effortless-rulebook/expense-approval-rulebook.json`
5. Update `effortless.json` so `-o expense-approval-rulebook.json` and `airtabletorulebook` is `IsDisabled: true`

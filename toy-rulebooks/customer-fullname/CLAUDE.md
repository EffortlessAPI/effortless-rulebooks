# Customer FullName — Effortless Project

<!-- rulebook-authoritative-banner -->
## Authoritative Source: `effortless-rulebook/customer-fullname-rulebook.json` is HEAD

For this project, **`effortless-rulebook/customer-fullname-rulebook.json` is the single, authoritative source of truth.** Edit it directly. All other artifacts (Postgres SQL, Python, Go, Excel, OWL, etc.) are mechanically derived from it via `effortless build`.

### ERB-only — no Airtable wiring

This project has **no Airtable base and no `airtabletorulebook` transpiler**. The JSON is the only source. There is no pull, no push, no sync, no upstream. Do not add Airtable wiring here without explicit user consent — this project exists specifically as the canonical "rulebook-as-SSoT, no Airtable" example.

### Never silently revert `customer-fullname-rulebook.json`

**Treat `effortless-rulebook/customer-fullname-rulebook.json` as sacred.** It contains human-authored business rules and CANNOT be casually overwritten. Before running any command that could touch this file — `effortless build`, `effortless airtabletorulebook`, any transpiler that writes to it, any sync script, any `git checkout`/`git restore` against it — first run `git status` / `git diff` on it.
...
- This applies to every `**/effortless-rulebook/customer-fullname-rulebook.json` in this repo, not just the project-local one.
...
- **String concatenation**: Derive a customer's full name from first and last names
- **Calculated fields**: A simple but complete example of formula-driven data
- **Hello World complexity**: The minimum viable ontology showing ERB principles

## Rulebook

**Location:** `effortless-rulebook/customer-fullname-rulebook.json`
...
Edit `effortless-rulebook/customer-fullname-rulebook.json` directly, then rebuild:
...
| `CLAUDE.md` | This file |
| `effortless-rulebook/customer-fullname-rulebook.json` | The rulebook (SSoT) |
...
- **Hub-and-spokes**: the rulebook is the hub; Postgres, Python, etc. are spokes
...
Edit `customer-fullname-rulebook.json`: add a new top-level object with `schema` and `data` properties. Then `effortless build`.

### Add a calculated field

Edit the table's field list in `customer-fullname-rulebook.json`: add a field with `type: "formula"` or `"lookup"` or `"aggregation"`. Then `effortless build`.

### Change seed data

Edit the `data` section of a table in `customer-fullname-rulebook.json`. Then rebuild to regenerate answer keys and test files.

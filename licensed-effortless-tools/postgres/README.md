...
PostgreSQL is tested via `execution-substrates/effortless-postgres/`:

```bash
# Initialize the database (must be done first)
cd licensed-effortless-tools/postgres
./init-db.sh

# Run conformance tests (from execution-substrates/effortless-postgres/)
cd ../../execution-substrates/effortless-postgres
./take-test.sh
```

#!/bin/bash
# create-substrate-report.sh - Generate substrate-report.html for YAML substrate
...
SUBSTRATE_NAME = "yaml"
SUBSTRATE_TITLE = "YAML Execution Substrate"
SUBSTRATE_ICON = "📄"
...
schema_yaml = read_file('schema.yaml', '# No schema available')
...
schema_lines = len(schema_yaml.split('\n'))
log_escaped = html.escape(log_content)
schema_escaped = html.escape(schema_yaml)
        ...
        <div id="schema" class="tab-content">
            <div class="card">
                <div class="code-header">
                    <h2>schema.yaml</h2>
                    <div><span class="code-info">{schema_lines} lines</span><button class="copy-btn" onclick="copyCode('schema-code')">Copy</button></div>
                </div>
                <pre id="schema-code">{schema_yaml}</pre>
            </div>
        </div>
    </main>
    <script>

#!/bin/bash
# create-substrate-report.sh - Generate substrate-report.html for XLSX substrate
...
SUBSTRATE_NAME = "xlsx"
SUBSTRATE_TITLE = "XLSX Execution Substrate"
SUBSTRATE_ICON = "📗"
...
# Try to read xlsx file info
xlsx_size = "N/A"
try:
    xlsx_size = f"{os.path.getsize('rulebook.xlsx') / 1024:.1f} KB"
except:
    pass
...
.file-card {{ display: flex; align-items: center; gap: 1rem; padding: 1rem; background: var(--bg-tertiary); border-radius: var(--radius); }}
.file-icon {{ font-size: 2.5rem; }}
.file-info h3 {{ font-size: 1rem; margin-bottom: 0.25rem; }}
.file-meta {{ font-size: 0.8rem; color: var(--text-secondary); }}
.formula-example {{ background: var(--code-bg); padding: 0.75rem; border-radius: var(--radius); margin-top: 1rem; font-family: monospace; font-size: 0.8rem; }}
    </style>
</head>
<body>
    <header>
    ...
    <nav class="tabs">
        ...
        <button class="tab" data-tab="workbook">Workbook Info</button>
        <button class="tab" data-tab="formulas">Formula Mapping</button>
    </nav>
    <main>
        <div id="description" class="tab-content active">
            <div class="card">
                ...
                <div class="markdown-content">
                    <p>The XLSX substrate generates native Excel workbooks with embedded formulas, allowing the rulebook logic to execute directly within Microsoft Excel.</p>
                    <h3>Orchestration Pattern</h3>
                    <ol>
                        <li><strong>Generate</strong>: <code>inject-into-xlsx.py</code> creates Excel workbook using openpyxl</li>
                        <li><strong>Inject Formulas</strong>: Converts rulebook formulas to Excel cell references</li>
                        <li><strong>Test</strong>: Opens workbook and validates calculated values match expected</li>
                    </ol>
                </div>
            </div>
            <div class="card">
                ...
                <ul class="markdown-content">
                    <li><strong>Native Excel Formulas</strong>: Calculations run directly in Excel</li>
                    <li><strong>Cell Reference Translation</strong>: <code>{{{{FieldRef}}}}</code> becomes <code>$B2</code>, etc.</li>
                    <li><strong>Multi-Sheet Support</strong>: Each entity gets its own worksheet</li>
                    <li><strong>Portable</strong>: Works offline without any dependencies</li>
                </ul>
            </div>
        </div>
        ...
        <div id="workbook" class="tab-content">
            <div class="card">
                ...
                <div class="file-card">
                    ...
                    <div class="file-info">
                        <h3>rulebook.xlsx</h3>
                        <div class="file-meta">Size: {xlsx_size}</div>
                    </div>
                </div>
...
output = html_template.format(
    ...
    log_content=log_escaped, xlsx_size=xlsx_size
)

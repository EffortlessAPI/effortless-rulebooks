#!/bin/bash
# create-substrate-report.sh - Generate substrate-report.html for OWL substrate
...
SUBSTRATE_NAME = "owl"
SUBSTRATE_TITLE = "OWL/SHACL Execution Substrate"
SUBSTRATE_ICON = "🦉"
...
ontology = read_file('ontology.owl', '# No ontology available')
individuals = read_file('individuals.ttl', '# No individuals available')
rules = read_file('rules.shacl.ttl', '# No rules available')
...
ontology_lines = len(ontology.split('\n'))
individuals_lines = len(individuals.split('\n'))
rules_lines = len(rules.split('\n'))

log_escaped = html.escape(log_content)
ontology_escaped = html.escape(ontology)
individuals_escaped = html.escape(individuals)
rules_escaped = html.escape(rules)
...
<body>
    ...
    <nav class="tabs">
        ...
        <button class="tab" data-tab="ontology">Ontology (TBox)</button>
        <button class="tab" data-tab="individuals">Individuals (ABox)</button>
        <button class="tab" data-tab="rules">SHACL Rules</button>
    </nav>
    <main>
        <div id="description" class="tab-content active">
            <div class="card">
                ...
                <div class="markdown-content">
                    <p>The OWL substrate generates semantic web artifacts: an OWL ontology (TBox), instance data (ABox), and SHACL-SPARQL rules for calculated fields.</p>
                    <h3>Orchestration Pattern</h3>
                    <ol>
                        <li><strong>Generate</strong>: <code>inject-into-owl.py</code> creates OWL/RDF files</li>
                        <li><strong>Ontology</strong>: <code>ontology.owl</code> defines classes and properties</li>
                        <li><strong>Individuals</strong>: <code>individuals.ttl</code> contains instance data</li>
                        <li><strong>Rules</strong>: <code>rules.shacl.ttl</code> defines SHACL-SPARQL calculation rules</li>
                        <li><strong>Reason</strong>: pyshacl executes rules to compute derived values</li>
                        <li><strong>Test</strong>: Compare reasoned values against expected answers</li>
                    </ol>
                </div>
            </div>
            <div class="card">
                ...
                <ul class="markdown-content">
                    <li><strong>Semantic Web Standards</strong>: OWL 2, RDF, SHACL, SPARQL</li>
                    <li><strong>Formal Reasoning</strong>: SHACL-SPARQL rules for inference</li>
                    <li><strong>Open World</strong>: Semantic web reasoning semantics</li>
                    <li><strong>Interoperability</strong>: Compatible with any RDF tooling</li>
                </ul>
            </div>
        </div>
        ...
        <div id="ontology" class="tab-content">
            <div class="card">
                <div class="code-header">
                    <h2>ontology.owl</h2>
                    <div><span class="code-info">{ontology_lines} lines</span><button class="copy-btn" onclick="copyCode('ontology-code')">Copy</button></div>
                </div>
                <pre id="ontology-code">{ontology}</pre>
            </div>
        </div>
        <div id="individuals" class="tab-content">
            <div class="card">
                <div class="code-header">
                    <h2>individuals.ttl</h2>
                    <div><span class="code-info">{individuals_lines} lines</span><button class="copy-btn" onclick="copyCode('individuals-code')">Copy</button></div>
                </div>
                <pre id="individuals-code">{individuals}</pre>
            </div>
        </div>
        <div id="rules" class="tab-content">
            <div class="card">
                <div class="code-header">
                    <h2>rules.shacl.ttl</h2>
                    <div><span class="code-info">{rules_lines} lines</span><button class="copy-btn" onclick="copyCode('rules-code')">Copy</button></div>
                </div>
                <pre id="rules-code">{rules}</pre>
            </div>
        </div>
    </main>
    <script>
...
output = html_template.format(
    ...
    ontology=ontology_escaped, individuals=individuals_escaped, rules=rules_escaped,
    ontology_lines=ontology_lines, individuals_lines=individuals_lines, rules_lines=rules_lines
)

#!/bin/bash
# create-substrate-report.sh - Generate substrate-report.html for Golang substrate
...
SUBSTRATE_NAME = "golang"
SUBSTRATE_TITLE = "Golang Execution Substrate"
SUBSTRATE_ICON = "🐹"
...
erb_sdk_go = read_file('erb_sdk.go', '// No generated code available')
main_go = read_file('main.go', '// No main.go available')
...
sdk_lines = len(erb_sdk_go.split('\n'))
main_lines = len(main_go.split('\n'))

log_escaped = html.escape(log_content)
sdk_escaped = html.escape(erb_sdk_go)
main_escaped = html.escape(main_go)
    ...
    <main>
        ...
        <div id="sdk" class="tab-content">
            <div class="card">
                <div class="code-header">
                    <h2>erb_sdk.go</h2>
                    <div><span class="code-info">{sdk_lines} lines</span><button class="copy-btn" onclick="copyCode('sdk-code')">Copy</button></div>
                </div>
        ...
        <div id="main" class="tab-content">
            <div class="card">
                <div class="code-header">
                    <h2>main.go</h2>
                    <div><span class="code-info">{main_lines} lines</span><button class="copy-btn" onclick="copyCode('main-code')">Copy</button></div>
                </div>
                <pre id="main-code">{main_go}</pre>
            </div>
        </div>
    </main>
    <script>
...
output = html_template.format(
    ...
    log_content=log_escaped, erb_sdk=sdk_escaped, main_go=main_escaped,
    sdk_lines=sdk_lines, main_lines=main_lines
)

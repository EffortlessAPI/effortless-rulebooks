...
| --- | --- | --- | --- | --- | --- |
| click-001 | count-number | home-card:entities | /developer/rulebook/entities | — | Click 'Entities: N' on Home → /rulebook/entities (the list of all N). |
| click-002 | count-number | home-card:substrates | /developer/substrates | — | Click 'Substrates: N' on Home → /substrates. |
| click-003 | count-number | home-card:spokes | /developer/spokes | — | Click 'Input spokes: N' on Home → /spokes. |
| click-004 | count-number | home-card:tests-passing | /developer/tests | status=passing | Click 'Tests passing: X/Y' on Home → /tests with passing filter. |
| click-005 | count-number | home-card:tests-failing | /developer/tests | status=failing | Click the failing-count chip on Home → /tests filtered to failing rows. |
...
| click-009 | formula-cell | entity-detail:schema-table | /developer/rulebook/formulas | entity=:entity&field=:field | Click a formula expression in the schema grid → /rulebook/formulas filtered to that formula. |
...
| click-014 | role-pill | users-list | /admin/users/roles | role=:roleId | Click a role pill anywhere → /users/roles focused on that role. |
| click-015 | role-pill | topbar:current-user | /admin/users/roles | role=:roleId | Click your own role pill in the top bar → see what that role gets / who else has it. |
